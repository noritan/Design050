...
void Clock_32k_Start(void) 
{
    ...
    Clock_32k_CLKEN |= Clock_32k_CLKEN_MASK;
	Clock_32k_CLKSTBY |= Clock_32k_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_32k_Stop
********************************************************************************
*
* Summary:
...
void Clock_32k_Stop(void) 
{
    ...
    Clock_32k_CLKEN &= (uint8)(~Clock_32k_CLKEN_MASK);
	Clock_32k_CLKSTBY &= (uint8)(~Clock_32k_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_32k_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_32k_CLKEN & Clock_32k_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_32k_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_32k_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_32k__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_32k_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_32k_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_32k_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_32k_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_32k_GetSourceRegister();
    uint16 oldDivider = Clock_32k_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_32k_CLKEN & Clock_32k_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_32k_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_32k_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_32k_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_32k_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_32k_MOD_SRC |= modeBitMask & (uint8)Clock_32k_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_32k_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_32k_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_32k_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_32k_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_32k_MOD_SRC & (uint8)(Clock_32k_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_32k_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_32k_GetDividerRegister();
    uint8 oldSrc = Clock_32k_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_32k_MOD_SRC |= CYCLK_SSS;
        Clock_32k_MOD_SRC =
            (Clock_32k_MOD_SRC & (uint8)(~Clock_32k_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_32k_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_32k_MOD_SRC & Clock_32k_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_32k_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_32k_PHASE = clkPhase & Clock_32k_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_32k_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_32k_PHASE & Clock_32k_PHASE_MASK;
}
