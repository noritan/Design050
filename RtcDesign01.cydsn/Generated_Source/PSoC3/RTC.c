...
/* Calculated sequence (31 * month )/ 12 ) mod 7 from the Zeller's congruence */
 const uint8 CYCODE RTC_monthTemplate[RTC_MONTHS_IN_YEAR] = \
...
void RTC_Stop(void) 
{
    ...
    RTC_OPPS_CFG_REG &= ~(RTC_OPPSIE_EN_MASK | RTC_OPPS_EN_MASK);
...
void RTC_DisableInt(void) 
{
    ...
    *
    *******************************************************************************/
    void RTC_DSTDateConversion(void) 
    {
        uint8 week = 1u;
        uint8 day = 1u;
        uint8 dayOfWeek;
...
void RTC_Init(void) 
{
    ...
    CyIntSetVector(RTC_ISR_NUMBER, RTC_ISR);
...
*
* Parameters:
*  timeDate: Pointer to RTC_TIME_DATE global stucture where new 
...
uint8 RTC_ReadStatus(void) CYREENTRANT
{
    ...
    RTC_statusDateTime &= ~RTC_STATUS_AA;
...
*
*******************************************************************************/
uint8 RTC_DayOfWeek(uint8 dayOfMonth, uint8 month, uint16 year) 
{
    ...
    */
    ...
    return ((uint8)((year + year/4 - year/100 + year/400 + RTC_monthTemplate[month-1] + dayOfMonth) % \
                    RTC_DAYS_IN_WEEK));
}
...
void RTC_SetInitValues(void) 
{
    ...
    /* Leap year check */
    if(RTC_IS_BIT_SET(RTC_statusDateTime, RTC_STATUS_LY))
    {
        /* Leap day check */
        if(!(RTC_currentTimeDate.DayOfMonth <= RTC_DAYS_IN_FEBRUARY + 1u &&
    ...
    else
    {
        ...
        * values
        */
        if(RTC_IS_BIT_SET(RTC_dstModeType, RTC_DST_RELDATE))
        {
        ...
        */   
        ...
        if(RTC_IS_BIT_SET(RTC_dstModeType, RTC_DST_ENABLE))
        {
            ...
            {
                if(RTC_IS_BIT_SET(RTC_dstStartStatus, RTC_DST_HOUR       | \
                                                                                RTC_DST_DAYOFMONTH | \
                                                                                RTC_DST_MONTH))  
                {
                    /* Add Hour and Min */
    ...
    *******************************************************************************/
    void RTC_WriteDSTMode(uint8 mode) 
    {
        ...
        if(RTC_IS_BIT_SET(mode,RTC_DST_RELDATE))
        {
            /* Convert DST date */
            RTC_DSTDateConversion();
        }
